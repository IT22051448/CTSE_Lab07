document.getElementById("checkBtn").addEventListener("click", async function () {
    ...
    try {
        ...
        output.textContent = "Status: " + response.status + "`n`n" + text;
    } catch (error) {
        output.textContent = "Request failed.`n`nThis may happen if the gateway is not running or CORS is not enabled.`n`n" + error;
    }
});